...
class KLKHFuelStationShowPage extends StatefulWidget {
  ...
  @override
  State<KLKHFuelStationShowPage> createState() =>
      _KLKHFuelStationShowPageState();
}
...
class _KLKHFuelStationShowPageState extends State<KLKHFuelStationShowPage>
  ...
  final Map<String, List<Map<String, String>>> categorizedChecklistItems = {
    'Lokasi Kerja': [
      {
        'key': 'PERMUKAAN_TANAH_RATA_CHECK',
        'label': 'Permukaan tanah rata dan tidak berlubang'
      },
      {
        'key': 'PERMUKAAN_TANAH_LICIN_CHECK',
        'label': 'Permukaan tanah tidak licin'
      },
      {
        'key': 'LOKASI_JAUH_LINTASAN_CHECK',
        'label': 'Lokasi kerja jauh dengan lintasan aktif angkutan'
      },
      {'key': 'TIDAK_CECERAN_B3_CHECK', 'label': 'Tidak ada ceceran B3'},
      {
        'key': 'PARKIR_FUELTRUCK_CHECK',
        'label': 'Area parkir khusus Fuel Truck untuk penyetokan tersedia'
      },
      {
        'key': 'PARKIR_LV_CHECK',
        'label': 'Area parkir khusus untuk LV tersedia'
      },
      {
        'key': 'LAMPU_KERJA_CHECK',
        'label':
            'Semua lampu kerja menyala dengan normal dan memadai untuk kerja malam hari'
      },
      {
        'key': 'FUEL_GENSET_CHECK',
        'label': 'Sisa fuel genset >10% kapasitas tangki'
      },
      {
        'key': 'AIR_BERSIH_TANDON_CHECK',
        'label': 'Sisa air dalam tandon air bersih >30% kapasitas tandon'
      },
    ],
    'Perlengkapan Kerja': [
      {
        'key': 'SOP_JSA_CHECK',
        'label': 'Tersedia SOP/ JSA untuk pekerjaan yang akan di lakukan'
      },
      {
        'key': 'SAFETY_POST_CHECK',
        'label':
            'Terpasang safety post sebagai batas berhenti unit untuk refueling'
      },
      {
        'key': 'RAMBU_APD_CHECK',
        'label': 'Terpasang rambu peringatan dan rambu APD'
      },
      {
        'key': 'PERLENGKAPAN_KERJA_CHECK',
        'label': 'Perlengkapan kerja ditata dengan rapi & tidak berserakan'
      },
      {'key': 'APAB_APAR_CHECK', 'label': 'Tersedia APAB dan APAR'},
      {'key': 'P3K_EYEWASH_CHECK', 'label': 'Tersedia kotak P3K dan Eyewash'},
      {
        'key': 'INSPEKSI_APAR_CHECK',
        'label': 'Terdapat tag inspeksi APAR dan eyewash yang sudah di inspeksi'
      },
      {
        'key': 'FORM_CHECKLIST_REFUELING_CHECK',
        'label': 'Tersedia form checklist peralatan Refueling'
      },
      {
        'key': 'TEMPAT_SAMPAH_CHECK',
        'label': 'Tersedia tiga wadah / tempat penampung sampah'
      },
    ],
    'Kegiatan Refueling Unit A2B': [
      {
        'key': 'MINEPERMIT_CHECK',
        'label': 'Fuelman memiliki dan membawa minepermit sebagai izin kerja'
      },
      {
        'key': 'SIMPER_OPERATOR_CHECK',
        'label':
            'Operator Fuel Truck memiliki dan membawa SIMPER sesuai peralatan yang digunakan'
      },
      {
        'key': 'PADLOCK_CHECK',
        'label': 'Tersedia Padlock untuk kegiatan refueling'
      },
      {
        'key': 'WADAH_PENAMPUNG_CHECK',
        'label': 'Tersedia wadah penampung untuk kegiatan Refueling'
      },
      {'key': 'WHEEL_CHOCK_CHECK', 'label': 'Tersedia ganjal / Wheel Chock'},
      {'key': 'RADIO_KOMUNIKASI_CHECK', 'label': 'Tersedia Radio Komunikasi'},
      {
        'key': 'APD_STANDAR_CHECK',
        'label': 'Pekerja memakai APD standar dan APD tambahan jika di perlukan'
      },
    ],
  };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Preview Fuel Station',
          ...
          : detailData == null
              ...
              : SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('Detail Inspeksi Fuel Station',
                          style: TextStyle(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              InfoRow(
                                  label: 'PIT',
                                  value: ': ${detailData!['PIT']}'),
                              InfoRow(
                                  label: 'Tanggal',
                                  value:
                                      ': ${formatTanggal(detailData!['DATE'])}'),
                              InfoRow(
                                  label: 'Waktu',
                                  value:
                                      ': ${formatWaktu(detailData!['TIME'])}'),
                              InfoRow(
                                  label: 'Shift',
                                  value: ': ${detailData!['SHIFT']}'),
                              InfoRow(
                                  label: 'PIC',
                                  value: ': ${detailData!['NAMA_PIC']}'),
                              InfoRow(
                          ...
                          children: [
                            ...
                            ListView(
                              ...
                              children:
                                ...
                                final key = item['key']!;
                                final value = detailData![
                                    key]; // nilai: 'true', 'false', atau 'n/a'
                                final noteKey =
                                    key.replaceFirst('_CHECK', '_NOTE');
                          ...
                          ? Column(
                              ...
                              children: [
                                ...
                                Center(
                                  child: QrImageView(
                                    data:
                                        'http://planner.ptsims.co.id/verified/${base64.encode(utf8.encode(detailData!['DIKETAHUI']))}',
